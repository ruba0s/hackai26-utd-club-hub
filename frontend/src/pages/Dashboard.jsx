...
      try {
        const token = await getIdToken();
        const res   = await fetch(`${API_URL}/api/events/calendar/${year}/${month}`, { headers: { Authorization: `Bearer ${token}` } });
        const data  = await res.json();
...
          const matchedClub = followedClubs.find(c => c.clubName?.toLowerCase().trim() === orgLower || orgLower.includes(c.clubName?.toLowerCase().trim()));
          const color       = isAiRec ? Y : matchedClub ? (clubColorMap[matchedClub.clubId] || "#888") : "#888";
          return { ...ev, _isAiRec: isAiRec, _color: color, _emoji: isAiRec ? "✦" : "📌", _day: new Date(ev.startTime).getDate() };
        });
        setEvents(enriched);
      } catch (err) {
...
        const matchedClub = followedClubs.find(c => c.clubName?.toLowerCase().trim() === ev.organizationName?.toLowerCase().trim());
...
                  return (
                    <CalCell key={day} day={day} events={dayEvs} selected={selectedDay===day} isToday={day===todayDate} onClick={() => setSelectedDay(day===selectedDay ? null : day)} />
                  );
                })}
              </div>
            </div>

            <RightPanel day={selectedDay} month={month} year={year} events={dayEvents} onClose={() => setSelectedDay(null)} />
          </div>
        </div>
      </div>
